// Tool contract test.
//
// The README promises one tool with a specific name and required parameters. The upstream list
...
//
// The last test calls the tool for real. Listing tools accepts any non-empty key, so a contract
...
const ENDPOINT = 'https://mcp.hasdata.com/api/mcp?apis=google_travel_hotels';
const KEY = process.env.HASDATA_API_KEY;
const TIMEOUT_MS = 30_000;

const EXPECTED = {
    hasdata_google_travel_hotels_getGoogleHotels: ['q', 'checkInDate', 'checkOutDate'],
};
...
test('apis=google_travel_hotels exposes exactly one tool', live, async () => {
    const tools = await listTools();
    const names = tools.map((t) => t.name).sort().join(', ');
    assert.equal(
        tools.length,
        Object.keys(EXPECTED).length,
        `expected ${Object.keys(EXPECTED).length} tool, got ${tools.length}: ${names}`
    );
});
...
test('the tool still declares its required parameter', live, async () => {
    const tools = await listTools();
...
test('the key is accepted by HasData', live, async () => {
    // Dates are computed at run time. A hardcoded future date silently becomes a past date, and
    // Google then rejects the stay, turning this canary red for the wrong reason.
    const day = (offset) => new Date(Date.now() + offset * 86_400_000).toISOString().slice(0, 10);
    const { raw } = await rpc('tools/call', {
        name: 'hasdata_google_travel_hotels_getGoogleHotels',
        // 'hotels in Barcelona' rather than 'Barcelona': a bare city name is read against
        // Google's own guess at where the search runs from, which in CI is a US datacentre.
        arguments: { q: 'hotels in Barcelona', checkInDate: day(30), checkOutDate: day(33), adults: 2 },
    });
